...
# from calculate_old_alg import main_alg as calculate
from calculate_new_alg import optimal_cutting as calculate
...
class MyWidget(QMainWindow):
    ...
    def get_res(self):
        self.output.clear()
        self.collect_data()
        cutter_len = int('0' + self.cutter_len.text())
        for i in self.data:
            res = calculate(i, self.pieces, cutter_len).tubes[:-1]
            res = [x - cutter_len for x in res]
            remainder = i - sum(res) - cutter_len * (len(res) - 1)
            print(res, remainder)
            self.draw(res, i, remainder)
            for j in res:
                self.pieces.remove(j)

    def draw(self, data, size, remainder):
        if data:
            tmp1, tmp2 = '', ''
            for i in data:
            ...
            self.output.append(
                ' ' * floor((87 - len(str(size))) / 2) + str(size) + ' ' * ceil((87 - len(str(size))) / 2))
            self.output.append("\n")
        else:
